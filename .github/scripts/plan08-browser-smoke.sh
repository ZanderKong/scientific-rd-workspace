...
run_code "const scope = page.locator('select[aria-label=\"Project Scope\"]'); if (!(await scope.isVisible())) { await page.locator('[data-sidebar=trigger]').click(); } await scope.waitFor({state: 'visible'}); const scopeState = await page.evaluate(() => Array.from(document.querySelectorAll('select')).map((select) => ({label: select.getAttribute('aria-label'), visible: Boolean(select.offsetWidth || select.offsetHeight), options: Array.from(select.options).map((option) => ({value: option.value, text: option.textContent}))}))); if (!scopeState.some((select) => select.label === 'Project Scope' && select.options.some((option) => option.text?.includes('${project_code}')))) throw new Error('Project Scope debug: ' + JSON.stringify(scopeState)); await page.evaluate((code) => { const select = document.querySelector('select[aria-label=\"Project Scope\"]'); if (!(select instanceof HTMLSelectElement)) throw new Error('Project Scope select is missing'); const option = Array.from(select.options).find((candidate) => candidate.textContent?.includes(code)); if (!option) throw new Error('Seeded project option is missing'); select.value = option.value; select.dispatchEvent(new Event('change', {bubbles: true})); }, '${project_code}'); await page.waitForTimeout(500)"
