...
run_code "const strip = page.getByTestId('resource-token-strip'); const tokens = strip.locator('[data-testid^=resource-token-]'); if (await tokens.count() !== 2) throw new Error('expected mixed Material and Equipment tokens'); const classes = await Promise.all([tokens.nth(0).getAttribute('class'), tokens.nth(1).getAttribute('class')]); if (!classes.some((value) => value?.includes('amber')) || !classes.some((value) => value?.includes('sky'))) throw new Error('semantic token colors missing'); const text = await strip.innerText(); if (text.includes('原料') || text.includes('设备')) throw new Error('token strip has category heading')"
