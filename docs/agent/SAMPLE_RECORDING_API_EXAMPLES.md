...
Example desired-state update (change a Process-owned temperature and the actual amount used for an existing Material):

```json
{
  "sample": { "title": "KI chlorine strip — corrected", "status": "active" },
  "steps": [
    {
  ...
  "change_note": "corrected as-run temperature and amount"
}
```

An existing Sample used as a precursor is referenced as a resource on the consuming Process. It is not copied and its ownership is not changed:

```json
{
  "target_object_id": "PRECURSOR_SAMPLE_UUID",
  "role": "precursor",
  "usage_values": {}
}
```

The same resource draft can include a new Equipment usage field and its current value in one transaction:
