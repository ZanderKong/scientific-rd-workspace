  ...
  useEffect(() => {
    api
      .listObjects({
        ...
        tag: '样品',
        q: query || undefined,
        project_scope_id: activeProjectId ?? undefined,
        include_global: true,
        limit: 100
      })
      .then(setItems)
    ...
    <main className='mx-auto w-full max-w-[1320px] px-4 py-7 md:px-8 md:py-10'>
      <div className='mb-6 flex items-start justify-between gap-3'>
        <div>
          ...
          <h1 className='mt-2 text-3xl font-semibold'>Samples</h1>
          <p className='mt-2 text-sm text-muted-foreground'>
            Samples are unified Research Objects grouped by tags.
          </p>
        </div>
        <Link
      ...
      <input
        ...
        placeholder='Search sample title, code, tags…'
      />
      {error ? (
        ...
        <div className='grid gap-3 md:grid-cols-2'>
          {items.map((item) => (
            <Link
              key={item.id}
              href={`/dashboard/samples/${item.id}`}
              className='rounded-2xl border bg-card/80 p-5 hover:border-primary'
            >
              <p className='font-mono text-[10px] text-muted-foreground'>{item.code}</p>
              <h2 className='mt-1 font-semibold'>{item.title}</h2>
              <div className='mt-3 flex flex-wrap gap-1'>
                {item.tags.map((tag) => (
                  <span key={tag} className='rounded bg-muted px-2 py-1 text-[10px]'>
                    {tag}
                  </span>
                ))}
              </div>
